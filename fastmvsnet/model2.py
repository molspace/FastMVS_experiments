...
from fastmvsnet.networks2 import *
...
class FastMVSNet(nn.Module):
	...
	#add depth convnet
        self.coarse_depth_conv = DepthConv(img_base_channels)
########        modified img_base_channels*4 ot *8    ##########################
        self.coarse_vol_conv = VolumeConv(img_base_channels * 8, vol_base_channels)
...
##################### modified code<end>########################################
	    ...
	    #depth - 2D-CNN
            curr_depth = gt_depth_img[:, i, :, :, :]
            curr_depth_feature = self.coarse_depth_conv(curr_depth)["conv2"]
 	
            #concat
            curr_feature_map_concat = torch.cat((curr_feature_map, curr_depth_feature), 1)

            coarse_feature_maps.append(curr_feature_map_concat)

        feature_list = torch.stack(coarse_feature_maps, dim=1)

        feature_channels, feature_height, feature_width = list(curr_feature_map_concat.size())[1:]
        depths = []
